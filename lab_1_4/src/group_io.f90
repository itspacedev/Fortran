...
contains
    ...
    ! Recursive function Get Avg Age (Lab 1.4) - Current lab
    pure recursive function Get_Avg_Age(Gender_Group, i, Cyear, Sum_Age) result(Avg_Age)
        type(student), intent(in)   :: Gender_Group(:)
        integer, intent(in)         :: Sum_Age, i, Cyear
        integer                     :: Summa, Avg_Age

        if (i == 0) then
            Avg_Age = Ceiling(Real(Sum_Age / Size(Gender_Group), R_))
        else
            Summa = Sum_Age + (Cyear - Gender_Group(i)%Year)
            Avg_Age = Get_Avg_Age(Gender_Group, i-1, Cyear, Summa)
        endif
